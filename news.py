...
tfidf_vectorizer = joblib.load('models/newsTfidfVectorizer.sav')

# Load the pre-trained ensemble model
ensemble_model = joblib.load('models/newsmodel.sav')
...
def news_main():
    ...
    with col1:
        st.image(image, caption='Spam Detection in News Post', width=200)
    with col2:
        # Input text box
        user_input = st.text_area("Enter an News Post:",height=300)

    if st.button("Predict"):
        if user_input.strip() == "":
            st.warning("Please enter an News Post.")
        else:
