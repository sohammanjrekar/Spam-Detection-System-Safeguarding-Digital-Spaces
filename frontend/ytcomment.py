...
tfidf_vectorizer = joblib.load('models/youtubeTfidfVectorizer.sav')

# Load the pre-trained ensemble model
ensemble_model = joblib.load('models/youtubemodel.sav')
...
def main():
    st.title("Spam Detection in Youtube Comments")
    ...
    with col1:
        st.image(image, caption='Spam Detection in Youtube Comments', width=200)
    with col2:
        # Input text box
    ...
    if st.button("Predict"):
        if user_input.strip() == "":
            st.warning("Please enter a Youtube Comments.")
        else:
